  ...
  for(i=0;i<numBricks;i++)
    {
      N=i*18;
      if(!colbricks[N+1]&&!colbricks[N+2])
	if(bricks[N+1]-0.1-fall<=-3.6&&bricks[N]>=-3+bluePos&&bricks[N]<=-2.4+bluePos&&bricks[N+1]-0.1-fall>-3.9)Over=1;
      if(!colbricks[N+2])
	continue;
...
void checkGreen()
{
  ...
  for(i=0;i<numBricks;i++)
	...
	if(bricks[N+1]-0.1-fall<=-3.6&&bricks[N]>=2.4+greenPos&&bricks[N]<=3+greenPos&&bricks[N+1]-0.1-fall>-3.9)Over=1;
      if(!colbricks[N+1])
	continue;
      if(bricks[N+1]-0.1-fall<=-3.6&&bricks[N]>=2.4+greenPos&&bricks[N]<=3+greenPos&&bricks[N+1]-0.1-fall>-3.9)
	{
	  for( int k=0;k<18;k++)
	    bricks[N+k]=0;
	  score+=speed/0.0001;
